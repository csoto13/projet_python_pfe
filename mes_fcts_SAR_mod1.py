...
"""Saisonnalité des différentes épidémies"""


def seasonal_SAR(y, t0, tau, T, p, Is_0, Sr_star_0, Js_0, Jr_0, c1, c2, params_mod):
    """Renvoit les solutions du modèle à la fin de la période souhaitée"""
    """y          : nombre d'année à simuler
       t0, tau, T : temps initial, durée été, durée année
       _0         : conditions initiales du modèle
       p          : proportion de résistants
       c1, c2     : coût de virulene sur pahtogène virulent, production frome de survies pathogène virulent
       params_mod : Lambda, Theta, mu, beta, delta, gamma, rho, pi_s"""

    # paramètres du modèle
    Lambda, Theta, mu, beta, delta, gamma, rho, pi_s = params_mod

    # paramètres des modèles été et hiber
    ...
    for k in range(y):
        ...
        Pv_saut = pi_s * (1 - c2) * int_SAR_sum[:, 4][-1] + pi_s * (1 - c2) * int_SAR_sum[:, 5][-1]
        ...
        Sr_init = Sr_0 * np.exp(- (1 / Lambda) * (Theta * Pa_0 + Theta * (1 - c1) * Pv_0))
        Is_init = ((Theta * Pa_0 * Ss_0) / (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)) * (
                    1 - np.exp(- (1 / Lambda) * (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)))
        Sr_star_init = ((Theta * Pa_0 * Sr_0) / (Theta * Pa_0 + rho * Theta * (1 - c1) * Pv_0)) * (
                    np.exp(- (1 / Lambda) * (1 - rho) * Theta * (1 - c1) * Pv_0) - np.exp(
                -(1 / Lambda) * (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)))
        Js_init = ((Theta * (1 - c1) * Pv_0 * Ss_0) / (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)) * (
                    1 - np.exp(- (1 / Lambda) * (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)))
        Jr_init = Sr_0 - (Sr_0 / (Theta * Pa_0 + rho * Theta * (1 - c1) * Pv_0)) * (
                    Theta * Pa_0 * np.exp(- (1 / Lambda) * (1 - rho) * Theta * (1 - c1) * Pv_0) + rho * Theta * (
                        1 - c1) * Pv_0 * np.exp(- (1 / Lambda) * (Theta * Pa_0 + Theta * (1 - c1) * Pv_0)))
